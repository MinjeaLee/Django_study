...
class Main(APIView):
    def get(self, request):
        ...
        for feed in feed_object_list:
            ...
            like_count = Like.objects.filter(feed_id = feed.id, is_like=True).count() # user's user_id
...
class ToggleLike(APIView):
    def post(self, request):
        feed_id = request.data.get("feed_id", None)
        favorite_status = request.data.get("favorite_status", True)
...
class ToggleBookmark(APIView):
    def post(self, request):
        feed_id = request.data.get("feed_id", None)
        bookmark_status = request.data.get("bookmark_status", True)

        if(bookmark_status == 'bookmark_border'):
            is_marked = True
        else:
            is_marked = False
